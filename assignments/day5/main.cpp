...
int main(){
    ...
    for (size_t i = 0; i < 6; i++) 
        for (size_t j = i + 1; j < 6; j++) {
            if (*shapes[i] > *shapes[j]) std::cout << i << " is greater than " << j << " - ";
            if (*shapes[i] == *shapes[j]) std::cout << i << " is equal than " << j << " - ";
            if (*shapes[i] < *shapes[j]) std::cout << i << " is less than " << j << " - ";

            //---------- my stuff 

            compare_more(*shapes[i], *shapes[j]);
            compare_equal(*shapes[i], *shapes[j]);
            compare_less(*shapes[i], *shapes[j]);
